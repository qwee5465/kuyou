...
class OutstockController extends BaseController
{
        ...
        if(IS_POST){
            ...
            if($this->checkFrom($list)){
                ...
                if($arr_result['resultcode'] == 0){
                    M()->commit(); //事务提交  
                    if($status==1){
                        $this->auditPass($osid,$create_time);
                    }else{
                        $this->ajaxReturn($arr_result);
                    }
                }else{
    ...
    public function outstockEdit(){
        $wid = getWid();
        $osid =I("osid");
        $sql ="select ifnull(h.num1,0) - g.num1 stock_num,f.brand_name,aa.remark remarkf,aa.total totalf,aa.create_time,aa.cid,aa.osname,bb.`name` cname,b.`name` gname,aa.total,d.unit_name unit_name1,a.num1*a.price*a.than totalx,a.* FROM db_out_stock aa left join db_client bb on aa.cid = bb.c_id left join db_out_stock_detail a on a.osid = aa.osid LEFT JOIN db_wholesale_goods bbb ON bbb.wgid = a.wgid LEFT JOIN db_goods b ON bbb.gid = b.gid Left JOIN db_brand f ON b.brand_id =f.brand_id LEFT JOIN db_unit d ON a.nei_unit_id = d.unit_id LEFT JOIN (select a.wgid,sum(a.num1)+sum(a.cd_num) as num1 from db_out_stock_detail a LEFT JOIN db_stock b on a.wgid = b.wgid where a.osid ='".$osid."' GROUP BY a.wgid) g ON g.wgid = a.wgid LEFT JOIN db_stock h on a.wgid=h.wgid WHERE a.osid = '".$osid."'  order by a.out_id asc"; 
        $list = M("")->query($sql); 
        foreach ($list as $k => $v) { 
            $sql ="select a.unit_id1,a.num,b.unit_name uname from db_spec a left join db_unit b on a.unit_id1=b.unit_id where a.wgid = ".$v['wgid'];
            $ulist = M()->query($sql);
            if($ulist){
...
//        dump($osbList);
    ...
    public function outstockRecord()
    {
        ...
        if($gname){  
            foreach($list as $k => $v) {
                $sql1 ="select b.wgid from db_out_stock_detail a left join db_wholesale_goods b on a.wgid=b.wgid left join db_goods c on b.gid = c.gid  where a.osid='".$v['osid']."' and c.name like '%".$gname."%'"; 
                $result = M("")->query($sql1); 
                if($result){
