    ...
    public function goodsAdd()
    {
        ...
        if(IS_POST){
            ...
            if($result){  
                ...
                $sql ="select gid from db_goods where `name`='".$name."'  and  brand_id = $brand_id";  
                $result = $goods->query($sql);  
                if($result){ //标准库中有该商品，批发商商品档案中没该商品 则添加
                    $data_goods = array(
                        'wid'=>$wid,
                        'gid'=>$result[0]['gid'],
                        'gtid'=>$gtid,
                    ...
                    // addActionLog("在商品档案中新增了一个商品,商品名为：【".$name."】");
                    $hid_spec = intval(I("hid_spec"));
                    if($hid_spec>0){
                        //新增商品规格 
                        for($i =0;$i<$hid_spec;$i++){
                            $num0 =I("num".$i); $unit_id0 = I("unit_id".$i); 
                    ...
                    // 标准库无该商品，标准库中添加一条信息,批发商档案填家一条信息 
                    $price = I('price');
                    $data = array(
                        'name'=>$name,
                        'gtid'=>$gtid,
                    ...
                    $list = $goods->add($data);
                    // addActionLog("新增了一个商品,商品名为：【".$name."】");
                    if($list){
                        $data_goods = array(
                            'wid'=>$wid,
                            'gid'=>$list,
    ...
    { 
        ...
        if(IS_POST){
            ...
            if($result){  
                ...
                //1.判断商品是否已审核
                $sql ="select b.`status`,b.gid from db_wholesale_goods a LEFT JOIN db_goods b on a.gid = b.gid where a.wgid = $wgid";
                $glist = M()->query($sql);
                $resultArr =array(
                    'data'=>0,
                    'msg'=>"成功"
                ...
                if($glist){
                    ...
                    if($status==1){ //未审核 
                        ...
                        //4.已审核判断商品名称和品牌 标准库中是否存在 
                        $result = $goods->field('gid')->where("name = '".$name."' and brand_id = $brand_id")->find(); 
                        if($result){
                            //5.存在  提取商品编号修改批发商商品信息
                            $wgdata = array(
                                'gid'=>$result['gid'],
                                'gtid'=>$gtid,
                                'gtid1'=>$gtid1,
                            ...
                            if($result<=0){
                                $resultArr['data']=1;
                                $resultArr['msg']="修改失败,在修改批发商商品信息时失败了2."; 
                            } 
                        }else{
                            //6.不存在 新增商品标准库和批发商信息
                            $gdata = array(
                            ...
                            if($result){
                                ...
                                if($result<=0){ 
                                    $resultArr['data']=1;
                                    $resultArr['msg']="修改失败,在添加批发商商品信息时失败了."; 
                                } 
                            }else{  
                                $resultArr['data']=1;
                                $resultArr['msg']="添加失败,在添加商品信息时失败了.";
                            }
                        } 
                    } 
                    $hid_spec = intval(I("hid_spec")); 
                            ...
                            if($isquote!=""){
                                if($isquote==1){//可编辑   
                                    ...
                                    if($num0>0 && $unit_id0>0){ 
                                        ...
                                        // if($result<=0){
                                        //     $resultArr['data']=1;
                                        //     $resultArr['msg']="修改失败,在修改商品规格$i时失败了.";  
                                        // }
                                    } 
                                }
                            }else{//新增
            ...
            //页面信息
            $wgid = I("wgid");
            $sql ="select a.wgid,b.`name` gname,a.unit_id,a.price,a.pic,b.brand_id,a.gtid,a.gtid1,a.gtid2,b.`status` from db_wholesale_goods a LEFT JOIN db_goods b on a.gid = b.gid WHERE a.wgid = $wgid";
            $list = M()->query($sql);
            $unit = M('unit');      //商品单位
            $brand = M('brand');    //品牌 
            $goods_type = M('goods_type'); //商品类型表 
